...
epiplot(Fatalities, x=Fatalities$service, y=Fatalities$income, graph = "bargroup", fill = Fatalities$breath) 

 ggplot(Fatalities, aes(x=service, y=income, fill=breath)) +
  geom_bar(position="dodge", stat="identity") + scale_fill_brewer(palette = "Paired")+theme_bw()+facet_wrap(~breath)

 ggplot(data, aes(x=x, y=y, fill=fill)) +
  geom_bar(position="dodge", stat="identity") + scale_fill_brewer(palette = "Paired")+theme_bw()+facet_wrap(~fill)
ggplot(data=Fatalities, aes(income)) +                     #for histograms - write function



 change colors to continuous scale
epiplot(data = test, x=test$jail ,graph = "bar")
...
ggplot(data=data1.1, aes(x=jail, y=drinkage)) +geom_bar(stat="identity")+facet_wrap(data1.1$jail)
