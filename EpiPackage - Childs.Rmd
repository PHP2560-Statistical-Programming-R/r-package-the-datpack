...
epiplot<- function(data, x, y, graph, fill, title, xlab, ylab, legend){    ###add argument for error bars
  ...
  barplot(table1, beside = TRUE, legend = levels(data$x), col=c("lightblue","darkblue"),main="title", xlab="xlab", ylab = "ylab")  
  ...
  boxplot(x~y, data=data, notch=TRUE,
  main="title", xlab="xlab", ylab="ylab")
} else if(graph=="hist"){
...
data1.1<-  Fatalities[complete.cases(Fatalities), ]


pic = graph & return(pic within each step - change colors to continuous scale
epiplot(data = data1.1, x=breath,graph = "bar")
