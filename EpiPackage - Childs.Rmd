...
epiplot<- function(data, x, y, graph, fill, title, xlab, ylab, legend){    ###add argument for error bars
  ...
  pic<-boxplot(x~y, data=data, notch=TRUE,
  main="title", xlab="xlab", ylab="ylab")
  return(pic)
} else if(graph=="hist"){
  pic<-ggplot(data=data, aes(x)) +                     #for histograms - write function
  geom_histogram(col="black", aes(fill=..count..)) +
  scale_fill_gradient("Count", low="light blue", high="navy")+
  labs(title="title", x="xlab", y="ylab")
  return(pic)
} else if(graph=="histcurve"){                      
...
epiplot(Fatalities, Fatalities$income, graph = "histcurve")
...
pic = graph & return(pic within each step - change colors to continuous scale
