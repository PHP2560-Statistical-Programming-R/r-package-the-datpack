...
map_prevalence <- function(data,state,year,existing_cases,population) {
  ...
  return(prevalence)
   #t<- all years or one year
     if (state == "al") {
 plot_usmap(data = Fatalities, values = "prevalence", include ="AL", lines = "black") + 
...
map_prevalence <- function(data,state,year,existing_cases,population) {
  data %>%
    group_by(state,year) %>%
    summarise(prevalence = existing_cases / population) 
 return(prevalence) 
 map<-plot_usmap(data = Fatalities, values = "prevalence", include ="state", lines = "black") + 
  scale_fill_continuous(low = "white", high = "dark blue", name = "fatalities", label = scales::comma) + labs(title = "Alabama") + theme(legend.position = "right")
 return(map)    
 }
