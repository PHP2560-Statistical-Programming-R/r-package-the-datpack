...
```{r}
map_region_prevalence <- function(data,existing_cases,population,state,year,region) {
  data %>%
    group_by(!! sym(state), !! sym(year)) %>%
    mutate(prevalence = !! sym(existing_cases) / !!sym(population)) %>%
...
map_region_prevalence(Fatalities,existing_cases= "fatal", population="pop", state="state", year="year", region == "Northeast")
```
